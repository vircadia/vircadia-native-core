...
MyAvatar::MyAvatar(QThread* thread) :
    ...
    _scriptedMotorMode(SCRIPTED_MOTOR_SIMPLE_MODE), _motionBehaviors(AVATAR_MOTION_DEFAULTS),
    _characterController(std::shared_ptr<MyAvatar>(this)), _eyeContactTarget(LEFT_EYE),
    _realWorldFieldOfView("realWorldFieldOfView", DEFAULT_REAL_WORLD_FIELD_OF_VIEW_DEGREES),
    _useAdvancedMovementControls("advancedMovementForHandControllersIsChecked", true), _showPlayArea("showPlayArea", true),
    _smoothOrientationTimer(std::numeric_limits<float>::max()), _smoothOrientationInitial(), _smoothOrientationTarget(),
    _hmdSensorMatrix(), _hmdSensorOrientation(), _hmdSensorPosition(), _recentModeReadings(MODE_READINGS_RING_BUFFER_SIZE),
    _bodySensorMatrix(), _goToPending(false), _goToSafe(true), _goToFeetAjustment(false), _goToPosition(), _goToOrientation(),
    _prevShouldDrawHead(true), _audioListenerMode(FROM_HEAD),
    ...
    _hmdAvatarAlignmentTypeSetting(QStringList() << AVATAR_SETTINGS_GROUP_NAME << "hmdAvatarAlignmentType",
                                   DEFAULT_HMD_AVATAR_ALIGNMENT_TYPE),
    ...
    _avatarEntityCountSetting(QStringList() << AVATAR_SETTINGS_GROUP_NAME << "avatarEntityData"
                                            << "size",
                              0),
    ...
    _analogPlusWalkSpeedSetting(QStringList() << AVATAR_SETTINGS_GROUP_NAME << "analogPlusWalkSpeed",
                                _analogPlusWalkSpeed.get()),
    ...
    connect(DependencyManager::get<AddressManager>().data(), &AddressManager::locationChangeRequired, this,
            static_cast<SlotType>(&MyAvatar::goToFeetLocation));
...
void MyAvatar::update(float deltaTime) {
    ...
    if (getControllerPoseInAvatarFrame(controller::Action::LEFT_HAND).isValid() &&
        getControllerPoseInAvatarFrame(controller::Action::RIGHT_HAND).isValid() && !(spine2Index < 0)) {
        ...
        if (glm::length(glm::vec2(handHipAzimuthSensorSpace.x, handHipAzimuthSensorSpace.z)) > 0.0f) {
            normedHandHipAzimuthSensorSpace =
                glm::normalize(glm::vec2(handHipAzimuthSensorSpace.x, handHipAzimuthSensorSpace.z));
            glm::vec2 headFacingPlusHandHipAzimuthMix =
                lerp(normedHandHipAzimuthSensorSpace, _headControllerFacing, PERCENTAGE_WEIGHT_HEAD_VS_SHOULDERS_AZIMUTH);
    ...
    if (newHeightReading.isValid()) {
        ...
        _averageUserHeightSensorSpace =
            lerp(_averageUserHeightSensorSpace, newHeightReading.getTranslation().y, HEIGHT_FILTER_COEFFICIENT);
    ...
    if (getControllerPoseInAvatarFrame(controller::Action::HEAD).getTranslation().y <
            (headDefaultPositionAvatarSpace.y - SQUAT_THRESHOLD) &&
        (angleSpine2 > COSINE_THIRTY_DEGREES) && (getUserRecenterModel() != MyAvatar::SitStandModelType::ForceStand)) {
    ...
    if (_drawAverageFacingEnabled) {
        ...
        glm::vec3 handAzimuthMidpoint =
            transformPoint(getTransform().getMatrix(), glm::vec3(_hipToHandController.x, 0.0f, _hipToHandController.y));
    ...
    if (getIsInWalkingState() && glm::length(getControllerPoseInAvatarFrame(controller::Action::HEAD).getVelocity()) <
                                     DEFAULT_AVATAR_WALK_SPEED_THRESHOLD) {
    ...
    if (_cauterizationNeedsUpdate) {
        ...
        forEachChild([&](SpatiallyNestablePointer object) {
            ...
            if (isChildOfHead && !object->hasGrabs()) {
                ...
                object->forEachDescendant(
                    [&](SpatiallyNestablePointer descendant) { updateChildCauterization(descendant, !_prevShouldDrawHead); });
                ...
                object->forEachDescendant(
                    [&](SpatiallyNestablePointer descendant) { objectsToUncauterize.insert(descendant); });
            }
        });

        // Redisplay cauterized entities that are no longer children of the avatar.
        for (auto cauterizedChild = objectsToUncauterize.begin(); cauterizedChild != objectsToUncauterize.end();
             cauterizedChild++) {
...
glm::vec2 MyAvatar::computeHandAzimuth() const {
    ...
    if (leftHandPoseAvatarSpace.isValid() && rightHandPoseAvatarSpace.isValid() && headPoseAvatarSpace.isValid() &&
        ...
        if ((glm::length(glm::vec2(rightHandSpine2Space.x, rightHandSpine2Space.z)) > 0.0f) &&
            (glm::length(glm::vec2(leftHandSpine2Space.x, leftHandSpine2Space.z)) > 0.0f)) {
            latestHipToHandController = lerp(glm::normalize(glm::vec2(rightHandSpine2Space.x, rightHandSpine2Space.z)),
                                             glm::normalize(glm::vec2(leftHandSpine2Space.x, leftHandSpine2Space.z)), HALFWAY);
        } else {
...
void MyAvatar::overrideHandAnimation(bool isLeft, const QString& url, float fps, bool loop, float firstFrame, float lastFrame) {
    if (QThread::currentThread() != thread()) {
        QMetaObject::invokeMethod(this, "overrideHandAnimation", Q_ARG(bool, isLeft), Q_ARG(const QString&, url),
...
void MyAvatar::resizeAvatarEntitySettingHandles(uint32_t maxIndex) {
    ...
    while (settingsIndex <= maxIndex) {
        Setting::Handle<QUuid> idHandle(QStringList() << AVATAR_SETTINGS_GROUP_NAME << "avatarEntityData"
                                                      << QString::number(settingsIndex) << "id",
                                        QUuid());
        ...
        Setting::Handle<QByteArray> dataHandle(QStringList() << AVATAR_SETTINGS_GROUP_NAME << "avatarEntityData"
                                                             << QString::number(settingsIndex) << "properties",
                                               QByteArray());
...
void MyAvatar::storeAvatarEntityDataPayload(const QUuid& entityID, const QByteArray& payload) {
    ...
    _avatarEntitiesLock.withWriteLock([&] { _cachedAvatarEntityBlobsToAddOrUpdate.push_back(entityID); });
}
...
void MyAvatar::clearAvatarEntity(const QUuid& entityID, bool requiresRemovalFromTree) {
    ...
    _avatarEntitiesLock.withWriteLock([&] { _cachedAvatarEntityBlobsToDelete.push_back(entityID); });
}
...
void MyAvatar::handleChangedAvatarEntityData() {
    ...
    if (getID().isNull() || getID() == AVATAR_SELF_ID || DependencyManager::get<NodeList>()->getSessionUUID() == QUuid()) {
    ...
    for (const auto& id : entitiesToDelete) {
        entityTree->withWriteLock([&] { entityTree->deleteEntity(id); });
    }
    ...
    for (const auto& id : entitiesToAdd) {
        ...
        if (blobFailed) {
            ...
            _avatarEntitiesLock.withWriteLock([&] { removeAllInstancesHelper(id, _cachedAvatarEntityBlobUpdatesToSkip); });
            continue;
        }
...
bool MyAvatar::updateStaleAvatarEntityBlobs() const {
    ...
    for (const auto& id : staleBlobs) {
        ...
        if (found) {
            ...
            _avatarEntitiesLock.withWriteLock([&] { _cachedAvatarEntityBlobs[id] = blob; });
        }
    }
    return true;
}
...
AvatarEntityMap MyAvatar::getAvatarEntityData() const {
    ...
    _avatarEntitiesLock.withReadLock([&] { result = _cachedAvatarEntityBlobs; });
    return result;
}
    ...
    _avatarEntitiesLock.withReadLock([&] {
        if (!_cachedAvatarEntityBlobs.empty()) {
            ...
            while (itr != _cachedAvatarEntityBlobs.end()) {
                ...
                QUuid id = _avatarEntityForRecording.size() == _cachedAvatarEntityBlobs.size()
                               ? _avatarEntityForRecording.values()[entityCount++]
                               : itr.key();
...
static float lookAtCostFunction(const glm::vec3& myForward,
                                const glm::vec3& myPosition,
                                const glm::vec3& otherForward,
                                const glm::vec3& otherPosition,
                                bool otherIsTalking,
                                bool lookingAtOtherAlready) {
    ...
    if (distance > GREATEST_LOOKING_AT_DISTANCE || myAngle > MAX_MY_ANGLE || otherAngle > MAX_OTHER_ANGLE) {
        ...
        return (DISTANCE_FACTOR * distance + MY_ANGLE_FACTOR * myAngle + OTHER_ANGLE_FACTOR * otherAngle +
                OTHER_IS_TALKING_TERM + LOOKING_AT_OTHER_ALREADY_TERM);
    }
}
...
void MyAvatar::computeMyLookAtTarget(const AvatarHash& hash) {
    ...
    foreach (const AvatarSharedPointer& avatarData, hash) {
        ...
        if (!avatar->isMyAvatar() && avatar->isInitialized()) {
            ...
            float cost =
                lookAtCostFunction(myForward, myPosition, otherForward, otherPosition, otherIsTalking, lookingAtOtherAlready);
            if (cost < bestCost) {
                bestCost = cost;
                bestAvatar = avatar;
            }
...
void MyAvatar::setJointData(const QString& name, const glm::quat& rotation, const glm::vec3& translation) {
    ...
    writeLockWithNamedJointIndex(name, [&](int index) { setJointData(index, rotation, translation); });
}
...
void MyAvatar::setJointRotation(const QString& name, const glm::quat& rotation) {
    ...
    writeLockWithNamedJointIndex(name, [&](int index) { setJointRotation(index, rotation); });
}
...
void MyAvatar::setJointTranslation(const QString& name, const glm::vec3& translation) {
    ...
    writeLockWithNamedJointIndex(name, [&](int index) { setJointTranslation(index, translation); });
}
...
void MyAvatar::clearJointData(const QString& name) {
    ...
    writeLockWithNamedJointIndex(name, [&](int index) { clearJointData(index); });
}
...
bool isWearableEntity(const EntityItemPointer& entity) {
    return entity->isVisible() && (entity->getParentID() == DependencyManager::get<NodeList>()->getSessionUUID() ||
...
void MyAvatar::attach(const QString& modelURL,
                      ...
                      bool useSaved) {
    if (QThread::currentThread() != thread()) {
        BLOCKING_INVOKE_METHOD(this, "attach", Q_ARG(const QString&, modelURL), Q_ARG(const QString&, jointName),
                               Q_ARG(const glm::vec3&, translation), Q_ARG(const glm::quat&, rotation), Q_ARG(float, scale),
                               Q_ARG(bool, isSoft), Q_ARG(bool, allowDuplicates), Q_ARG(bool, useSaved));
        return;
    }
    AttachmentData data;
...
void MyAvatar::detachOne(const QString& modelURL, const QString& jointName) {
    if (QThread::currentThread() != thread()) {
        BLOCKING_INVOKE_METHOD(this, "detachOne", Q_ARG(const QString&, modelURL), Q_ARG(const QString&, jointName));
        return;
    }
    QUuid entityID;
...
void MyAvatar::detachAll(const QString& modelURL, const QString& jointName) {
    if (QThread::currentThread() != thread()) {
        BLOCKING_INVOKE_METHOD(this, "detachAll", Q_ARG(const QString&, modelURL), Q_ARG(const QString&, jointName));
        return;
    }
    QUuid entityID;
...
void MyAvatar::setAttachmentsVariant(const QVariantList& variant) {
    if (QThread::currentThread() != thread()) {
        BLOCKING_INVOKE_METHOD(this, "setAttachmentsVariant", Q_ARG(const QVariantList&, variant));
        return;
    }
    ...
    if (!useAdvancedMovementControls() && qApp->isHMDMode()) {
        ...
        switch (_controlSchemeIndex) {
            ...
            case LocomotionControlsMode::CONTROLS_ANALOG_PLUS:
                if (zSpeed || xSpeed) {
                    glm::vec3 scaledForward = getSensorToWorldScale() * calculateGearedSpeed(zSpeed) * _walkSpeedScalar *
                                              ((zSpeed >= stickFullOn) ? getSprintSpeed() : getWalkSpeed()) * forward;
                    glm::vec3 scaledRight = getSensorToWorldScale() * calculateGearedSpeed(xSpeed) * _walkSpeedScalar *
                                            ((xSpeed > stickFullOn) ? getSprintSpeed() : getWalkSpeed()) * right;
...
bool MyAvatar::requiresSafeLanding(const glm::vec3& positionIn, glm::vec3& betterPositionOut) {
    ...
    auto findIntersection = [&](const glm::vec3& startPointIn, const glm::vec3& directionIn, glm::vec3& intersectionOut,
        ...
        EntityItemID entityID =
            entityTree->evalRayIntersection(startPointIn, directionIn, include, ignore,
                                            PickFilter(PickFilter::getBitMask(PickFilter::FlagBit::COLLIDABLE) |
                                                       PickFilter::getBitMask(PickFilter::FlagBit::PRECISE) |
                                                       PickFilter::getBitMask(PickFilter::FlagBit::DOMAIN_ENTITIES) |
                                                       PickFilter::getBitMask(
                                                           PickFilter::FlagBit::AVATAR_ENTITIES)),  // exclude Local entities
...
void MyAvatar::setDriveGear1(float shiftPoint) {
    ...
    if (shiftPoint > 1.0f || shiftPoint < 0.0f)
        return;
...
glm::mat4 MyAvatar::getSpine2RotationRigSpace() const {
    ...
    glm::mat4 spine2RigSpace(glm::vec4(w, 0.0f), glm::vec4(u, 0.0f), glm::vec4(v, 0.0f),
...
float MyAvatar::computeStandingHeightMode(const controller::Pose& head) {
    ...
    if (head.isValid()) {
        ...
        for (auto recentModeReadingsIterator = _recentModeReadings.begin();
             recentModeReadingsIterator != _recentModeReadings.end(); ++recentModeReadingsIterator) {
        ...
        if (!(modeInMeters > getCurrentStandingHeight())) {
            ...
            if (getResetMode() && getControllerPoseInAvatarFrame(controller::Action::HEAD).isValid()) {
                setResetMode(false);
                float resetModeInCentimeters =
                    glm::floor((head.getTranslation().y - MODE_CORRECTION_FACTOR) * CENTIMETERS_PER_METER);
...
static bool handDirectionMatchesHeadDirection(const controller::Pose& leftHand,
                                              const controller::Pose& rightHand,
...
static bool isHeadLevel(const controller::Pose& head, const glm::quat& averageHeadRotation) {
    ...
    return ((fabs(diffFromAverageEulers.x) < DEFAULT_HEAD_PITCH_STEPPING_TOLERANCE) &&
            (fabs(diffFromAverageEulers.z) < DEFAULT_HEAD_ROLL_STEPPING_TOLERANCE));
}
...
bool MyAvatar::FollowHelper::shouldActivateHorizontalCG(MyAvatar& myAvatar) const {
    ...
    if (myAvatar.getIsInWalkingState()) {
        ...
        if (!withinBaseOfSupport(currentHeadPose) && headAngularVelocityBelowThreshold(currentHeadPose) &&
            ...
            glm::vec3 defaultHipsPosition =
                myAvatar.getAbsoluteDefaultJointTranslationInObjectFrame(myAvatar.getJointIndex("Hips"));
            glm::vec3 defaultHeadPosition =
                myAvatar.getAbsoluteDefaultJointTranslationInObjectFrame(myAvatar.getJointIndex("Head"));
            ...
            if (!isActive(Horizontal) && (!isActive(Vertical)) &&
...
glm::mat4 MyAvatar::getCenterEyeCalibrationMat() const {
    ...
    if (rightEyeIndex >= 0 && leftEyeIndex >= 0) {
        ...
        return createMatFromQuatAndPos(DEFAULT_AVATAR_MIDDLE_EYE_ROT,
                                       extractTranslation(headMat) + DEFAULT_AVATAR_HEAD_TO_MIDDLE_EYE_OFFSET);
    }
}
...
const QUuid MyAvatar::grab(const QUuid& targetID,
                           int parentJointIndex,
                           glm::vec3 positionalOffset,
                           glm::quat rotationalOffset) {
    auto grabID = QUuid::createUuid();
    // create a temporary grab object to get grabData

    QString hand = "none";
    if (parentJointIndex == CONTROLLER_RIGHTHAND_INDEX || parentJointIndex == CAMERA_RELATIVE_CONTROLLER_RIGHTHAND_INDEX ||
        parentJointIndex == FARGRAB_RIGHTHAND_INDEX || parentJointIndex == getJointIndex("RightHand")) {
        hand = "right";
    } else if (parentJointIndex == CONTROLLER_LEFTHAND_INDEX || parentJointIndex == CAMERA_RELATIVE_CONTROLLER_LEFTHAND_INDEX ||
               parentJointIndex == FARGRAB_LEFTHAND_INDEX || parentJointIndex == getJointIndex("LeftHand")) {
        hand = "left";
    }

    Grab tmpGrab(DependencyManager::get<NodeList>()->getSessionUUID(), targetID, parentJointIndex, hand, positionalOffset,
                 rotationalOffset);
...
void MyAvatar::addAvatarHandsToFlow(const std::shared_ptr<Avatar>& otherAvatar) {
    if (QThread::currentThread() != thread()) {
        QMetaObject::invokeMethod(this, "addAvatarHandsToFlow", Q_ARG(const std::shared_ptr<Avatar>&, otherAvatar));
        return;
    }
 ...
 */
void MyAvatar::useFlow(bool isActive,
                       bool isCollidable,
                       const QVariantMap& physicsConfig,
                       const QVariantMap& collisionsConfig) {
    if (QThread::currentThread() != thread()) {
        QMetaObject::invokeMethod(this, "useFlow", Q_ARG(bool, isActive), Q_ARG(bool, isCollidable),
                                  Q_ARG(const QVariantMap&, physicsConfig), Q_ARG(const QVariantMap&, collisionsConfig));
        return;
    }
    if (_skeletonModel->isLoaded()) {
        auto& flow = _skeletonModel->getRig().getFlow();
...
QVariantMap MyAvatar::getFlowData() {
    QVariantMap result;
    if (QThread::currentThread() != thread()) {
        BLOCKING_INVOKE_METHOD(this, "getFlowData", Q_RETURN_ARG(QVariantMap, result));
        return result;
    }
    if (_skeletonModel->isLoaded()) {
...
QVariantList MyAvatar::getCollidingFlowJoints() {
    QVariantList result;
    if (QThread::currentThread() != thread()) {
        BLOCKING_INVOKE_METHOD(this, "getCollidingFlowJoints", Q_RETURN_ARG(QVariantList, result));
        return result;
    }
