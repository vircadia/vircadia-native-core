...
function mouseMove(event) {
    ...
    // brad's hack to move an overlay to the mouse
    //Overlays.editOverlay(pointerOverlay, { position: lookAt3D });
    /*
    var pickRay = Camera.computePickRay(event.x, event.y);
    var result = Overlays.findRayIntersection(pickRay);
    print(JSON.stringify(result));
    if (result.intersects) {
        Overlays.editOverlay(pointerOverlay, { position: result.intersection });
        Reticle.setVisible(false);
        print("intersects...");
    } else {
        var lookAt3D = HMD.getHUDLookAtPosition3D();
        Overlays.editOverlay(pointerOverlay, { position: { x: 0, y: 0, z: 0 } });
...
var intersectionBeaconPosition = { x: 0, y: 0, z: 0 };
var intersectionBeaconSize = 0.03;
var intersectionBeacon = Overlays.addOverlay("sphere", {
                    position: intersectionBeaconPosition,
                    size: intersectionBeaconSize,
...
Script.scriptEnding.connect(function(){
    Overlays.deleteOverlay(intersectionBeacon);
});
...
Script.update.connect(function(deltaTime) {
    ...
    if (lastMouseMoveEvent) {
        ...
        if (timeSinceLastDepthCheck > TIME_BETWEEN_DEPTH_CHECKS) {
            var pickRay = Camera.computePickRay(lastMouseMoveEvent.x, lastMouseMoveEvent.y);
            var result = Overlays.findRayIntersection(pickRay);
            ...
            if (result.intersects) {
                ...
                Overlays.editOverlay(intersectionBeacon, { position: pointAt3D, visible: false });
            }
